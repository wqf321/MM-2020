...
class Net:
    def __init__(self, args):
        ...
        if self.model_name == 'MMGCN':
            ...
            self.model = MMGCN(self.features, self.edge_index, self.user_index_5,self.batch_size, self.num_user, self.num_item, self.aggr_mode, self.concat, self.num_layer, self.has_id, self.dim_latent,self.weight_decay,pos_row, pos_col).to(self.device)
    ...
    def run(self):
        ...
        with open(f'./train_log_({args.l_r:.6f}_{args.weight_decay:.6f}).txt', "a") as f:
        ...
        for epoch in range(self.num_epoch):
            ...
            if torch.isnan(sum_loss/self.batch_size):
                with open('./Data/result_{0}_{1}_noselfloog_noact_noweight.txt'.format(args.l_r,args.weight_decay),
                          'a') as save_file:
                    save_file.write('lr: {0} \t Weight_decay:{1} is Nan'.format(args.l_r, args.weight_decay))
                break
            ...
            if test_recall > max_recall:
                ...
                if num_decreases > 20:
                    with open('./Data/result_{0}_{1}.txt'.format(args.l_r,args.weight_decay), 'a') as save_file:
                        save_file.write(
                            'lr: {0} \t Weight_decay:{1} =====> Precision:{2} \t Recall:{3} \t NDCG:{4}\r\n'.
                            format(args.l_r, args.weight_decay, max_precision, max_recall, max_NDCG))
                    break
                else:
                    num_decreases += 1
            # if epoch % 1 == 0:
